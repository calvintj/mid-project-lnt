...
<body>
    <div class="container mt-2">
        <div class="row">
            <div class="col-lg-12 margin-tb">
                <div class="pull-left mb-4 mt-5">
                    <h2>Edit Data Karyawan PT Meksiko</h2>
                </div>
            </div>
        </div>
        @if (session('status'))
        ...
        @endif
        <form class="col-lg" action="{{ route('karyawans.update', $karyawan->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-10">
                            <input type="number" name="umur" class="form-control" placeholder="Umur Karyawan" value="{{ $karyawan->umur }}"
                                id="inputUmur">
                        </div>
                        @error('umur')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-10">
                            <textarea type="text" name="alamat" class="form-control" placeholder="Alamat Karyawan"
                                id="inputAlamat">{{ $karyawan->alamat }}</textarea>
                        </div>
                        @error('alamat')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="telp" class="form-control" placeholder="No Telepon Karyawan" value="{{ $karyawan->telp }}"
                                id="inputTelp">
                        </div>
                        @error('telp')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
